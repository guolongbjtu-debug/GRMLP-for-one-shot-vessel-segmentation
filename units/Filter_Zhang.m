...
% x = [6,1,0.5,5,10];
% [Jerman_gray,Jerman_binary] = Filter_Zhang(x,img);
% figure()
% imshow(Jerman_gray)
% fprintf('Filter_Zhang没问题')


function  [Jerman_gray,Jerman_binary] = Filter_Zhang(X,img)
    ...
    threshold.adjust = beyond.adjust;  
    ...
    % 计算Vf输出图和性能
    output_gray = zhang_filter_main(Ip, 0.5:0.5:N, [1;1], tau, false);
    ...
    % [img_high_recall_gray,img_high_recall_binary] = zhang_filter(img,img,beyond.tau,threshold,beyond.N);
end


function vesselness = zhang_filter_main(I, sigmas, spacing, tau, brightondark)
    ...
    for j = 1:length(sigmas)
        ...
        gama = Lambda_rho/3;
        Rs = sqrt(Lambda1.*Lambda1 + Lambda2.*Lambda2 + Lambda_rho.*Lambda_rho);

        response = Lambda2.*Lambda2.*(Lambda_rho-Lambda2).* 27 ./ (Lambda2 + Lambda_rho).^3;    
        response = response.*(1-exp(-Rs.*Rs./(2*gama)));
